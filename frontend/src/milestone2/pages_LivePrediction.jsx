...
export default function LivePrediction() {
  ...
  return (
    <DashboardLayout pageTitle="Live Prediction">
      <div className="m2-content">
        ...
        <section className="m2-hero">
          <div>
            <div className="m2-eyebrow">REAL-TIME DETECTION</div>
            <h1>
              Run a <span>Live Prediction</span>
            </h1>
            <p>Submit a processed security event to the Isolation Forest + rules engine.</p>
          </div>
        </section>
        ...
        <form className="m2-panel m2-live-form" onSubmit={submit}>
          ...
          <button className="m2-primary" type="submit" disabled={busy}>
            {busy ? (
              <>
                <Loader2 size={16} className="m2-spin" /> Analyzing…
              </>
            ) : (
              "Analyze Event"
            )}
          ...
          {result && (
            <div className="m2-result-box">
              <b>{result.prediction}</b>
              <span>
                {result.severity} · {result.confidence_score}% confidence · {result.threat_type}
              </span>
              <div>
                {(result.reasons || []).map((reason, i) => (
                  <p key={`${result.event_id || "result"}-reason-${i}`}>• {reason}</p>
                ))}
              </div>
            </div>
          )}
